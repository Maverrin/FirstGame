...
inline UPackage* GetPackage(UVoxelMaterialCollection* Collection, const FString& Suffix)
{
	...
	if (ObjectsToDelete.Num() > 0)
	{
		...
		if (ObjectTools::DeleteObjects(ObjectsToDelete, false) != ObjectsToDelete.Num())
		{
		...
		for (auto& Material : Collection->Materials)
		{
			...
			else
			{
				for (auto& Child : Material.Children)
				{
					...
					StaticSwitchParameters.Add(FStaticSwitchParameter(FMaterialParameterInfo("IsTessellationEnabled"), bEnableTessellation, true, FGuid()));
...
private:
	...
	{
		(bEnableTessellation ? Collection->GeneratedSingleMaterialsTess : Collection->GeneratedSingleMaterials).Add(Index, Material);
	}
	inline void AddDoubleMaterial(FVoxelMaterialCollectionDoubleIndex Index, UMaterialInterface* Material)
	{
		(bEnableTessellation ? Collection->GeneratedDoubleMaterialsTess : Collection->GeneratedDoubleMaterials).Add(Index, Material);
	}
	inline void AddTripleMaterial(FVoxelMaterialCollectionTripleIndex Index, UMaterialInterface* Material)
	{
		(bEnableTessellation ? Collection->GeneratedTripleMaterialsTess : Collection->GeneratedTripleMaterials).Add(Index, Material);
	}
	...
	inline UMaterialInstanceConstant* CreateMaterialInstance(FString AssetName, UMaterialInterface* Parent)
	{
		...
		auto* Result = Factory->FactoryCreateNew(UMaterialInstanceConstant::StaticClass(), Package, FName(*(AssetName + GetTessellationSuffix())), RF_Public | RF_Standalone, NULL, GWarn);
	...
	{
		UMaterial* NewTemplate = DuplicateObject<UMaterial>(Template, Package, FName(*(Template->GetName() + "_" + InSuffix + GetTessellationSuffix())));
		NewTemplate->AtomicallySetFlags(RF_Public | RF_Standalone);
	...
	{
		UMaterialFunction* NewFunction = DuplicateObject<UMaterialFunction>(MaterialFunctionToDuplicate, Package, FName(*(Prefix + MaterialFunctionToDuplicate->GetName() + GetTessellationSuffix())));
		NewFunction->AtomicallySetFlags(RF_Public | RF_Standalone);
...
bool FVoxelMaterialCollectionHelpers::GenerateSingleMaterials(UVoxelMaterialCollection* Collection, FString& OutError)
{
	...
	Collection->GeneratedSingleMaterials.Reset();
	Collection->GeneratedSingleMaterialsTess.Reset();
...
bool FVoxelMaterialCollectionHelpers::GenerateDoubleMaterials(UVoxelMaterialCollection* Collection, FString& OutError)
{
	...
	Collection->GeneratedDoubleMaterials.Reset();
	Collection->GeneratedDoubleMaterialsTess.Reset();
	UEditorLoadingAndSavingUtils::SavePackages({ Collection->GetOutermost() }, false);
...
bool FVoxelMaterialCollectionHelpers::GenerateTripleMaterials(UVoxelMaterialCollection* Collection, FString& OutError)
{
	...
	Collection->GeneratedTripleMaterials.Reset();
	Collection->GeneratedTripleMaterialsTess.Reset();
	UEditorLoadingAndSavingUtils::SavePackages({ Collection->GetOutermost() }, false);
