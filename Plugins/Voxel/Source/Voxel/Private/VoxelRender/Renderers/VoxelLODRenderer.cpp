...
FAsyncOctreeBuilderTask::FAsyncOctreeBuilderTask(uint8 LOD, uint8 LODLimit, const FIntBox& WorldBounds)
	: LOD(LOD)
	, LODLimit(LODLimit)
...
void FAsyncOctreeBuilderTask::Init(const TArray<FIntBox>& InCameraBounds, TSharedPtr<FVoxelChunkOctree> InOctree, const FRichCurve* InLODCurve)
{
	check(!bIsActive);

	CameraBounds = InCameraBounds;
	Octree = InOctree;
	if (InLODCurve)
	{
		LODCurve = MakeUnique<FRichCurve>();
		*LODCurve = *InLODCurve;
	}
	else
	{
...
void FAsyncOctreeBuilderTask::DoWork()
{
	...
	Settings.LODCurve = LODCurve.Get();
	Settings.WorldBounds = WorldBounds;
	Settings.CameraBounds = CameraBounds;
...
{
	OldChunk.MoveGrassToArray(GrassMeshes);
}
...
FVoxelLODRenderer::~FVoxelLODRenderer()
{
	...
	if (OctreeBuilder.IsValid() && OctreeBuilder->IsActive() && !Pool->OctreeBuilderPool->RetractQueuedWork(OctreeBuilder.Get()))
	{
...
void FVoxelLODRenderer::Tick(float DeltaTime)
{
...

	{
		...
		if (OctreeBuilder.IsValid() && OctreeBuilder->IsActive() && OctreeBuilder->IsDone())
		{
			bOctreeBuilt = true;
			UpdateLOD();
			OctreeBuilder->Reset();
...

	{
		...
		if (NextUpdateTime < Time && (!OctreeBuilder.IsValid() || !OctreeBuilder->IsActive()))
		{
			...
			if (!OctreeBuilder.IsValid())
			{
				OctreeBuilder = MakeUnique<FAsyncOctreeBuilderTask>(World->GetOctreeDepth(), World->GetLODLimit(), World->GetBounds());
			}
			auto* Curve = World->GetLODCurve();
			OctreeBuilder->Init(CameraBounds, Octree, Curve ? &Curve->FloatCurve : nullptr);
...
UVoxelProceduralMeshComponent* FVoxelLODRenderer::GetNewMesh(const FIntVector& Position, uint8 LOD)
{
	...
	{
		NewMesh = NewObject<UVoxelProceduralMeshComponent>(World, NAME_None, RF_Transient);
		...
		NewMesh->LightingChannels = World->GetLightingChannels();
...
void FVoxelLODRenderer::RemoveGrass(UHierarchicalInstancedStaticMeshComponent* Grass)
{
	if (IsValid(Grass)) // Else can crash sometimes when closing UE
	{
...
UHierarchicalInstancedStaticMeshComponent* FVoxelLODRenderer::GetNewGrass(const FIntVector& Position)
{
	...
	{
		NewGrass = NewObject<UHierarchicalInstancedStaticMeshComponent>(World, NAME_None, RF_Transient);
		NewGrass->SetupAttachment(World->GetRootComponent(), NAME_None);
		NewGrass->Mobility = EComponentMobility::Movable;
		NewGrass->RegisterComponent();
	}

	NewGrass->SetRelativeLocation(World->GetChunkRelativePosition(Position));
...
uint64 FVoxelLODRenderer::GetSquaredDistanceToInvokers(const FIntVector& Position) const
{
	...
	for (auto& Invoker : World->GetInvokers())
	{
		if (Invoker.IsValid())
		{
			Distance = FMath::Min<uint64>(Invoker->GetCameraBounds(World).ComputeSquaredDistanceFromBoxToPoint<uint64>(Position), Distance);
		}
	}
	return Distance;
}
...
void FVoxelLODRenderer::UpdateLOD()
{
...

	{
		...
		for (auto& NewChunkBounds : ChunksToCreate)
		{
			...
			if (World->GetChunkCullingLOD() < LOD || (World->IsDedicatedServer() && LOD != 0))
			{
				continue;
			}
