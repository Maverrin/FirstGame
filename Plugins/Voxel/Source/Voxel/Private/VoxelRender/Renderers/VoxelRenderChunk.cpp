...
void FVoxelRenderChunk::RecomputeMeshPosition()
{
	...
	{
		GrassMesh.Value->SetRelativeLocation(Render->World->GetChunkRelativePosition(Position));
	}
}
...
void FVoxelRenderChunk::Tick()
{
	...
	if (MeshTask.IsValid())
	{
		if (MeshTask->IsDone() && Render->CanApplyTask(TaskId))
		{
			...
			ApplyGrassAndActorsFromMeshTask();
...
void FVoxelRenderChunk::MoveGrassToArray(TArray<UHierarchicalInstancedStaticMeshComponent*>& OutGrass)
{
	for (auto& GrassMesh : GrassMeshes)
	{
		OutGrass.Add(GrassMesh.Value);
	}
	GrassMeshes.Reset();
}

void FVoxelRenderChunk::DestroyGrass()
{
	for (auto& GrassMesh : GrassMeshes)
	{
		Render->RemoveGrass(GrassMesh.Value);
	}
}

void FVoxelRenderChunk::ApplyGrassAndActorsFromMeshTask()
{	
	// Grass
	{
		PreviousGrassInfo = MeshTask->NewPreviousGrassInfo;
		// Make sure GrassMeshes has all the keys
		for (auto& BufferIt : MeshTask->GrassBuffers)
		{
			if (!GrassMeshes.Contains(BufferIt.Key))
			{
				auto NewGrass = Render->GetNewGrass(Position);
				FVoxelGrassUtilities::InitGrass(NewGrass, BufferIt.Value->GrassVariety);
				GrassMeshes.Add(BufferIt.Key, NewGrass);
			}
		}

		for (auto& GrassMeshIt : GrassMeshes)
		{
			auto& Grass = GrassMeshIt.Value;
			check(Grass);
			auto* Buffer = MeshTask->GrassBuffers.Find(GrassMeshIt.Key);
			if (Buffer)
			{
	...
	// Actors
	if (MeshTask->bComputeActors && !Render->World->HaveActorsBeenCreated(Bounds))
	{
		Render->World->NotifyActorsAreCreated(Bounds);
		Render->World->AddActorInfos(MeshTask->ActorsToSpawn);
	}
}
