...
template<typename T>
inline void GetGradient(T* Data, const FVoxelMap& Map, const FIntVector& P, FVector& Result)
{
	const int32& X = P.X;
	const int32& Y = P.Y;
	const int32& Z = P.Z;

	Result.X = Data->GetValue(X + 1, Y, Z, Map).ToFloat() - Data->GetValue(X - 1, Y, Z, Map).ToFloat();
	Result.Y = Data->GetValue(X, Y + 1, Z, Map).ToFloat() - Data->GetValue(X, Y - 1, Z, Map).ToFloat();
	Result.Z = Data->GetValue(X, Y, Z + 1, Map).ToFloat() - Data->GetValue(X, Y, Z - 1, Map).ToFloat();
}

template<class C>
inline void GetNormalImpl(const C* Data, const FVector& Position, const int& Step, const FVoxelMap& Map, FVector& Result)
{
	FIntVector A, B;
	GetClosestPoints(Position, A, B);

	FVector NA, NB;
	GetGradient(Data, Map, A, NA);
	GetGradient(Data, Map, B, NB);
...
bool FVoxelMCPolygonizer::CreateChunk(FVoxelChunk& OutChunk, FVoxelStatsElement& Stats)
{
	...
	Data->GetValuesAndMaterials(CachedValues, CachedMaterials, Bounds, Bounds.Min, LOD, ChunkDataSize, LOD);

	if (LOD > 0)
	{
		Stats.StartStat("GetMap");
		Data->GetMap(Bounds, Map);
	}
					...
					if (CaseCode != 0 && CaseCode != 255)
					{
						...
						for (int I = 0; I < CellData.GetVertexCount(); I++)
						{
							...
							if (!bIsVertexCached || VertexIndex == -1)
							{
								...
								{
									GetNormal(Vertex.Position, Vertex.GradientNormal);
...
void FVoxelMCPolygonizer::GetNormal(const FVector& Position, FVector& Result) const
{
	if (LOD == 0)
	{
		GetNormalImpl(this, Position, Step, Map, Result);
	}
	else
	{
		GetNormalImpl(Data, Position + (FVector)ChunkPosition, Step, Map, Result);
	}
}

FVoxelValue FVoxelMCPolygonizer::GetValue(int X, int Y, int Z, const FVoxelMap& InMap) const
{
		...
		if (TransitionsMask & Direction)
		{
			for (int LX = 0; LX < CHUNK_SIZE; LX++)
			{
				for (int LY = 0; LY < CHUNK_SIZE; LY++)
				{
					...
					CornerValues[0] = GetValue(Direction, (2 * LX + 0) * HalfStep, (2 * LY + 0) * HalfStep);
					CornerValues[1] = GetValue(Direction, (2 * LX + 1) * HalfStep, (2 * LY + 0) * HalfStep);
					CornerValues[2] = GetValue(Direction, (2 * LX + 2) * HalfStep, (2 * LY + 0) * HalfStep);
					CornerValues[3] = GetValue(Direction, (2 * LX + 0) * HalfStep, (2 * LY + 1) * HalfStep);
					CornerValues[4] = GetValue(Direction, (2 * LX + 1) * HalfStep, (2 * LY + 1) * HalfStep);
					CornerValues[5] = GetValue(Direction, (2 * LX + 2) * HalfStep, (2 * LY + 1) * HalfStep);
					CornerValues[6] = GetValue(Direction, (2 * LX + 0) * HalfStep, (2 * LY + 2) * HalfStep);
					CornerValues[7] = GetValue(Direction, (2 * LX + 1) * HalfStep, (2 * LY + 2) * HalfStep);
					CornerValues[8] = GetValue(Direction, (2 * LX + 2) * HalfStep, (2 * LY + 2) * HalfStep);
							...
							if (!bIsVertexCached || VertexIndex == -1)
							{
								...
								else
								{
									...
									while (Max - Min != 1)
									{
										...
										FVoxelValue ValueAtMiddle = GetValue(Direction, bIsAlongX ? Middle : PositionA.X, bIsAlongY ? Middle : PositionA.Y);
								...
								FVector FinalPoint = (EdgeIndex == 7 || EdgeIndex == 8 || EdgeIndex == 9) ? FVoxelRenderUtilities::GetTranslated(IntersectionPoint, Normal, TransitionsMask, LOD) : IntersectionPoint;
...
void FVoxelMCTransitionsPolygonizer::GetNormal(const FVector& Position, FVector& Result) const
{
	GetNormalImpl(Data, Position + (FVector)ChunkPosition, Step, Map, Result);
}
