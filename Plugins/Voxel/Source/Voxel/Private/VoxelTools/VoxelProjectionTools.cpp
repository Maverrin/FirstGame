	...
	EFailReason& FailReason)
{
	...
	for (auto& Hit : Work.HitVoxels)
	{
		...
		{
			float OldValue = Data->GetValue(Hit.Position, Octree).ToFloat();
			float NewValue;
			...
			Data->SetValue(Hit.Position, NewValue, Octree);
	...
	EFailReason& FailReason)
{
	...
	for (auto& Hit : Work.HitVoxels)
	{
		if (LIKELY(Data->IsInWorld(Hit.Position)))
		{
			...
			FVoxelMaterial Material = Data->GetMaterial(Hit.Position, Octree);
			...
			Data->SetMaterial(Hit.Position, Material, Octree);
	...
	FVoxelData* Data = World->GetData();
	{
		...
		for (auto& Hit : Work.HitVoxels)
		{
			if (LIKELY(Data->IsInWorld(Hit.Position)))
			{
				...
				Voxel.Position = Hit.Position;
				FVoxelValue Value;
				Data->GetValueAndMaterial(Hit.Position, Value, Voxel.Material, Octree);
