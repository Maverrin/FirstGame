...
struct Array1
{
   ...
   Array1(unsigned long n_)
      : n(0), max_n(0), data(0)
   {
      if(n_>ULONG_MAX/sizeof(T)) check(false);
   ...
   {
      assert(n_<=max_n_);
      if(max_n_>ULONG_MAX/sizeof(T)) check(false);
   ...
   void assign(unsigned long num, const T* copydata)
   {
      ...
      if(num>max_n){
         if(num>ULONG_MAX/sizeof(T)) check(false);
         std::free(data);
   ...
   {
      if(r>ULONG_MAX/sizeof(T)) check(false);
